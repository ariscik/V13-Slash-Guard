...
module.exports = {
    ...
    ws: false,
    run: async (Aris) => {
        let pic = await Aris.guild.fetchAuditLogs({ type: "CHANNEL_DELETE" }).then(a => a.entries.first());
        ...
        if (ibne.bannable) ibne.ban({ reason: `Aris Kanal Silme Koruması` });
        ytKapat(Aris.guild.id)
        if (logcuk) {
            logcuk.send({
                embeds: [new Discord.MessageEmbed().setFooter(`Kanalı kurmak için \`/kanalkur kanal: ${Aris.id}\``).setDescription(`
\`>\` ${pic.executor} \`(${pic.executor.id})\` kişisi tarafından kanal silindi!
